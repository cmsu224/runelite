...
import net.runelite.client.plugins.socket.plugins.socketDPS.SocketDpsConfig;
import net.runelite.client.plugins.socket.plugins.socketDPS.SocketDpsCounterPlugin;
    ...
    public Dimension render(Graphics2D graphics) {
        if (this.config.displayOverlay() && !this.plugin.getMembers().isEmpty() && this.client.getLocalPlayer() != null) {
            ...
            dpsMembers.forEach((k, v) -> {
                String right = QuantityFormatter.formatNumber((long)v.intValue());
                if (k.equalsIgnoreCase(this.client.getLocalPlayer().getName())) {
                    Color color = this.config.highlightSelf() ? Color.GREEN : Color.WHITE;
                    this.panelComponent.getChildren().add(LineComponent.builder().leftColor(color).left(k).rightColor(color).right(right).build());
                } else if (this.config.highlightOtherPlayer() && this.plugin.getHighlights().contains(k.toLowerCase())) {
                    this.panelComponent.getChildren().add(LineComponent.builder().leftColor(this.config.getHighlightColor()).left(k).rightColor(this.config.getHighlightColor()).right(right).build());
                } else {
                    this.panelComponent.getChildren().add(LineComponent.builder().left(k).right(right).build());
                }
            });
            ...
            if (localName != null && dpsMembers.containsKey(localName) && tot > dpsMembers.get(localName) && this.config.showTotal()) {
                this.panelComponent.getChildren().add(LineComponent.builder().leftColor(Color.RED).left("Total").rightColor(Color.RED).right(dpsMembers.get("Total").toString()).build());
            }
