...
public class InventoryWidgetItemQuery extends WidgetItemQuery
{
    ...
    private Collection<WidgetItem> getInventoryItems(Client client)
    {
        ...
        {
            Widget inventory = (Widget) client.getWidget(widgetInfo);
            if (inventory == null || inventory.isHidden())
            {
                continue;
            }

            Widget[] children = (Widget[]) inventory.getDynamicChildren();
            for (int i = 0; i < children.length; i++)
            {
                ...
                widgetItems.add(new WidgetItem(child.getItemId(), child.getItemQuantity(), i, bounds, (net.runelite.api.widgets.Widget) child, dragBounds));
            }
            break;
        }
        return widgetItems;
