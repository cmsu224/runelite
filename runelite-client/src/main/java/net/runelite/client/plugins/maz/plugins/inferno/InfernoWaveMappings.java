...
import net.runelite.client.plugins.maz.plugins.inferno.displaymodes.InfernoNamingDisplayMode;
...
class InfernoWaveMappings {
    ...
    static void addWaveComponent(InfernoConfig config, PanelComponent panelComponent, String header, int wave, Color titleColor, Color color) {
        int[] monsters = (int[])waveMapping.get(wave);
        if (monsters != null) {
            ...
            for(int i = 0; i < monsters.length; ++i) {
                ...
                int count;
                for(count = 1; i < monsters.length - 1 && monsters[i + 1] == monsterType; ++i) {
                    ++count;
                }

                TitleComponentBuilder builder = TitleComponent.builder();
                String npcNameText = "";
                if (config.npcNaming() == InfernoNamingDisplayMode.SIMPLE) {
                    npcNameText = npcNameText + (String)npcNameMappingSimple.get(monsterType);
                } else {
                    npcNameText = npcNameText + (String)npcNameMappingComplex.get(monsterType);
                }

                if (config.npcLevels()) {
                    npcNameText = npcNameText + " (" + monsterType + ")";
                }
    ...
    static {
        Builder<Integer, int[]> waveMapBuilder = new Builder();
        ...
        Builder<Integer, String> nameMapBuilderSimple = new Builder();
        ...
        Builder<Integer, String> nameMapBuilderComplex = new Builder();
