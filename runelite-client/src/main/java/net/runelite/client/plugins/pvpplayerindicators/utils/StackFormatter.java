...
public class StackFormatter {
    ...
    public static String quantityToStackSize(long quantity) {
        if (quantity < 0L) {
            return "-" + quantityToStackSize(quantity == -9223372036854775808L ? 9223372036854775807L : -quantity);
        } else if (quantity < 10000L) {
            ...
            for(int i = SUFFIXES.length - 1; i >= 0; --i) {
                divideBy = (long)Math.pow(10.0D, (double)(i * 3));
                if ((double)(quantity / divideBy) >= 1.0D) {
                    suffix = SUFFIXES[i];
                    break;
                }
            }
            ...
            String var10000 = formattedString.endsWith(".") ? formattedString.substring(0, 3) : formattedString;
    ...
    public static String quantityToRSStackSize(int quantity) {
        if (quantity == -2147483648) {
            ...
            return quantity < 10000000 ? quantity / 1000 + "K" : quantity / 1000000 + "M";
        }
    }
    ...
    public static String quantityToRSDecimalStack(int quantity, boolean precise) {
        ...
        if (quantityStr.length() <= 4) {
            ...
            int power = (int)Math.log10((double)quantity);
            NumberFormat format = precise && power >= 6 ? PRECISE_DECIMAL_FORMATTER : DECIMAL_FORMATTER;
            String var10000 = format.format((double)quantity / Math.pow(10.0D, (double)(power / 3 * 3)));
            return var10000 + SUFFIXES[power / 3];
        }
    }
            ...
            if (!suffix.equals("")) {
                for(int i = 1; i < SUFFIXES.length; ++i) {
                    if (SUFFIXES[i].equals(suffix.toUpperCase())) {
                        return (int)Math.pow(10.0D, (double)(i * 3));
                    }
                }
    ...
    static {
        NUMBER_FORMATTER = NumberFormat.getInstance(Locale.ENGLISH);
        DECIMAL_FORMATTER = new DecimalFormat("#,###.#", DecimalFormatSymbols.getInstance(Locale.ENGLISH));
        PRECISE_DECIMAL_FORMATTER = new DecimalFormat("#,###.###", DecimalFormatSymbols.getInstance(Locale.ENGLISH));
    }
}
