...
public class HighlightAnythingOverlay extends Overlay {
    private Client client;
    private HighlightAnythingPlugin plugin;
    ...
    @Inject
    public HighlightAnythingOverlay(Client client, HighlightAnythingPlugin plugin, HighlightAnythingConfig config) {
        this.client = client;
        this.plugin = plugin;
        this.config = config;
    ...
    public Dimension render(Graphics2D g) {
        ...
        Iterator graphicObjects = this.client.getGraphicsObjects().iterator();
        ...
        while(highLightObjects) {
            ...
            boolean debug;
            do {
                if (!graphicObjects.hasNext()) {
                    for(int x = 0; x < 104; ++x) {
                        for(int y = 0; y < 104; ++y) {
                            Tile t = client.getScene().getTiles()[client.getPlane()][x][y];
                            if (t != null) {

                                highLightDecor = parse_string(plugin.decorWhiteList, WorldPoint.fromLocalInstance(client, client.getLocalPlayer().getLocalLocation()).getRegionID());
                                if(highLightDecor || config.debugDecor()){
                ...
                go = (GraphicsObject)graphicObjects.next();
                debug = client.isKeyPressed(86) && client.isKeyPressed(81);
