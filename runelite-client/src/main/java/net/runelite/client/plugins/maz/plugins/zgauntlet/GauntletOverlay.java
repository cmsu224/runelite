package net.runelite.client.plugins.maz.plugins.zgauntlet;
...
public class GauntletOverlay extends Overlay
{
	...
	@Inject
	private GauntletOverlay(GauntletPlugin plugin, GauntletConfig config, ModelOutlineRenderer modelOutlineRenderer)
	{
		this.plugin = plugin;
		this.config = config;
    ...
    public Dimension render(Graphics2D graphics)
    {
    	...
    	if (config.hunleffAttackCounter() || config.hunleffTickCounter() || config.playerAttackCounter())
		{
			if (plugin.getHunleff() != null)
			{
				...
				{
					line1 += plugin.getHunleffAttackCounter();
				...
				{
					line2 += plugin.getPlayerAttackCounter();
				}
				Point canvasPointCenter = plugin.getHunleff().getCanvasTextLocation(graphics, "", 130);
				if (canvasPointCenter != null)
				{
					...
					Point line1Point = new Point(canvasPointCenter.getX() - width1 / 2, canvasPointCenter.getY()+30);
					Point line2Point = new Point(canvasPointCenter.getX() - width2 / 2,
							config.hunleffTickCounter() || config.hunleffAttackCounter() ? canvasPointCenter.getY()+30 + metrics.getHeight() : canvasPointCenter.getY()+30);
					...
					Color hunleffColor = plugin.getPlayerAttackColorOverwrite() == null ? plugin.getHunleffAttackColor() : plugin.getPlayerAttackColorOverwrite();
					if (this.config.hunleffOutlineOnPray() && this.client.getLocalPlayer() != null && this.client.getLocalPlayer().getWorldLocation().distanceTo(plugin.getHunleff().getWorldLocation()) <= 11) {
						if (hunleffColor == Color.CYAN && this.client.getLocalPlayer() != null && this.client.getLocalPlayer().getOverheadIcon() != HeadIcon.MAGIC) {
							this.modelOutlineRenderer.drawOutline(plugin.getHunleff(), 2, hunleffColor, 2);
						}else if (hunleffColor == Color.GREEN && this.client.getLocalPlayer().getOverheadIcon() != HeadIcon.RANGED) {
							this.modelOutlineRenderer.drawOutline(plugin.getHunleff(), 2, hunleffColor, 2);
						}
					}
				}
				...
				for (ResourceObject object : plugin.getResourceObjects().values())
				{
					if (client.getLocalPlayer().getWorldLocation() != null && (config.resourceDrawDistance() == 0 || object.getInstancePoint().distanceToWorldPoint(client, wp) <= config.resourceDrawDistance()))
					{
						...
						if (lp != null)
						{
							...
							if(config.resourceDrawDistance() > 0){
								if (poly != null)
								{
