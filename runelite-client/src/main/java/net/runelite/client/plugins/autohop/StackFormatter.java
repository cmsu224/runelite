...
public class StackFormatter {
    ...
    public static String quantityToStackSize(long quantity) {
        if (quantity < 0L) {
            ...
            for(int i = SUFFIXES.length - 1; i >= 0; --i) {
                ...
                if ((double)quantity / (double)divideBy >= 1.0D) {
                    suffix = SUFFIXES[i];
                    break;
                }
            }

            String formattedString = NUMBER_FORMATTER.format((double)quantity / (double)divideBy);
    ...
    public static String quantityToRSDecimalStack(int quantity, boolean precise) {
        ...
        if (quantityStr.length() <= 4) {
            ...
            return format.format((double)quantity / Math.pow(10.0D, (double)(power / 3 * 3))) + SUFFIXES[power / 3];
        }
    }
