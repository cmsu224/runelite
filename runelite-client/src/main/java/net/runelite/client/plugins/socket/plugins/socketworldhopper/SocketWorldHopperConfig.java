...
@ConfigGroup(value="socketworldhopper")
public interface SocketWorldHopperConfig
extends Config {
    ...
    @ConfigItem(keyName="showSidebar", name="Show world hopper sidebar", description="Show sidebar containing all worlds that mimics in-game interface", position=4)
    ...
    @ConfigItem(keyName="menuOption", name="Show Hop-to menu option", description="Adds Hop-to menu option to the friends list and clan members list", position=7)
    ...
    @ConfigItem(keyName="combatHop", name="Combat Hop", description="Queue up world hop to instantly hop after out of combat", position=10)
    default public boolean combatHop() {
        return false;
    }

    @ConfigItem(keyName="customWorldCycle", name="Custom world cycle", description="Hopping cycles between this list of worlds if present. Seperate with Comma. Compatible with socket plugin. Any player in socketcan set the current custom worlds by updating their config. ", position=11)
    default public String customWorldCycle() {
        return "";
    }

    @ConfigItem(position=12, keyName="hopperName", name="Hopper Name", description="Name of the player you're waiting to hop after")
    default public String getHopperName() {
        return "";
    }

    @ConfigItem(position=13, keyName="hopperName2", name="Hopper2 Name", description="Name of the player you're waiting to hop after")
    default public String getHopperName2() {
        return "";
    }

    @ConfigItem(position=14, keyName="hopSound", name="Hop Sound", description="Sound when world hopping")
    default public boolean playSound() {
        return true;
    }
}
